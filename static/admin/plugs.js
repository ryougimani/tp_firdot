...
define(['jquery'], function () {
	...
	function menu() {
		 ...
		 * @returns {string}
		 */
		this.parseUrl = function (url, obj) {
			var params = {};
			if (url.indexOf('?') !== -1) {
				...
				for (var i in queryParams) {
					if (queryParams[i].indexOf('=') !== -1) {
						...
						try {
							...
							console.log([e, url, queryParams, hash]);
						}
					}
				}
			}
			url = this.getUrl(url);
			params.spm = obj && obj.getAttribute('data-menu-node') || this.queryNode(url);
			if (!params.token) {
		...
		this.listen = function () {
			...
			window.onhashchange = function () {
				...
				if (!/^m\-[\d\-]+$/.test(node)) {
					node = $.menu.queryNode($.menu.getUrl()) || '';
				}
