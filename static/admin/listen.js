...
//define(['admin.plugs'], function () {

	$(function () {

		this.$body = $('body');

		// 注册 data-reload 事件 (刷新页面)
		this.$body.on('click', '[data-reload]', function () {
			$.form.reload();
		});

		// 注册 data-open 事件 (打开一个内置页面)
		this.$body.on('click', '[data-open]', function () {
			$.form.href($(this).attr('data-open'), this);
		});

		// 注册 data-right 事件 (打开一个左侧页面)
		this.$body.on('click', '[data-right]', function () {
			$.form.right($(this).attr('data-right'), 'open_type=right');
		});

		// 注册 data-right-close 事件 (关闭一个左侧页面)
		this.$body.on('click', '[data-right-close]', function () {
			$.form.hide('.framework-right');
		});

		// 注册 data-modal 事件 (打开一个到弹出层)
		this.$body.on('click', '[data-modal]', function () {
			return $.form.modal($(this).attr('data-modal'), 'open_type=modal', $(this).attr('data-title') || '编辑');
		});

		// 注册 data-update 事件行为 (列表页面的数据更改)
		this.$body.on('click', '[data-update]', function () {
			var id = $(this).attr('data-update') || (function () {
				var data = [];
				return $($(this).attr('data-list-target') || 'input.list-check').map(function () {
		...
		// 注册 data-check-all 事件 (全选)
		this.$body.on('click', '[data-check-all]', function () {
			var checked = !!this.checked, data_check = $(this).attr('data-check-all');
			$('[data-check=' + data_check + ']').map(function () {
				this.checked = checked;
			});
		});
		// 注册 data-check 事件 (选择)
		this.$body.on('click', '[data-check]', function () {
			var data_check = $(this).attr('data-check');
			$('[data-check-all=' + data_check + ']').prop('checked', true);
			$('[data-check]').map(function () {
				(!this.checked) && $('[data-check-all=' + data_check + ']').prop('checked', false);
			});
		});

		// 注册 data-icon 事件 (图标选择界面)
		...
		this.$body.on('submit', 'form.form-search', function () {
			...
			if ((this.method || 'get').toLowerCase() === 'get') {
				window.location.href = '#' + $.menu.parseUrl(url + split + $(this).serialize());
			}
		...
		this.$body.on('click', 'a[data-page-href]', function () {
			window.location.href = '#' + $.menu.parseUrl(this.href, this);
		});

		// 注册 data-file 事件行为 (上传文件界面)
		this.$body.on('click', '[data-file]', function () {
			...
			var url = window.ROOT_URL + '/index.php/admin/plugs/upload/mode/' + method + '.html?mode=' + method + '&upType=' + upType + '&type=' + type + '&field=' + field + '&model=' + model;
		...
		this.$body.on('click', '[data-tips-image]', function () {
			...
			img.onload = function () {
				layer.open({
					type: 1, area: imgWidth, title: false, closeBtn: 1, skin: 'layui-layer-nobg', shadeClose: true, content: $(img).appendTo('body').css({background: '#fff', width: imgWidth, height: 'auto'}), end: function () {
						$(img).remove();
					}
				});
			};
