...
class Pinyin {
	...
	public function __construct() {
		if (empty($this->characters)) {
			$this->characters = file_get_contents(dirname(__FILE__) . '/characters.dat');
		}
	}
	...
	*/
	public function transformWithTone($input_char, $delimiter = ' ', $outside_ignore = false) {
		...
		for ($i = 0; $i < $input_len; $i++) {
			...
			if (preg_match('/^[\x{4e00}-\x{9fa5}]$/u', $word) && preg_match('/\,' . preg_quote($word) . '(.*?)\,/', $this->characters, $matches)) {
